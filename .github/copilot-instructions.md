...
- All R scripts across both dataset folders (`syn52047893/` and `ERP161086/`)
...
- Shell scripts (Salmon, FASTQC, SUPPA2) and how they feed into R analyses
...
| Folder | Dataset | Sequencing | Quantification | Role in pipeline |
|---|---|---|---|---|
| `syn52047893/` | Long-read RNA-seq (Synapse: syn52065646) | Oxford Nanopore | Bambu | Primary DTU/DIU discovery via DEXSeq (IsoformSwitchAnalyzeR); also hosts the **targeted** splicing analysis (SUPPA2 + beta regression) restricted to the 86 genes with significant DIU |
| `ERP161086/` | Short-read RNA-seq (ENA: ERP161086) | Illumina paired-end, 150 bp | Salmon v0.14.2 | Independent cross-platform concordance check of AD-vs-CT effect sizes for the significant DIU gene set (not a standalone DTU pipeline) |

> ⚠️ An earlier whole-genome-wide short-read DTU/beta-regression pipeline (`PRJNA1206164/`) has been retired and is excluded from version control via `.gitignore`. Do not reference it as an active part of the pipeline unless the user explicitly asks about it.

### Core Tools & Libraries

- **IsoformSwitchAnalyzeR** — central Bioconductor package for DTU analysis, switch consequence analysis, and switch plotting
- **DEXSeq** — statistical testing backend for DTU (long-read)
- **Bambu** — long-read-aware transcript discovery and quantification
- **Salmon** (v0.14.2) — short-read transcript-level quantification for the ERP161086 concordance analysis
- **SUPPA2** — alternative splicing event quantification (PSI values)
- **BSgenome.Hsapiens.UCSC.hg38** — genome object for ORF analysis
- **betareg / glmmTMB** — beta regression for PSI-based differential splicing, restricted to transcripts from significant DIU genes
- **gprofiler2** — GO and pathway enrichment
- **ggplot2, ggrepel, ggpattern, patchwork, gridExtra** — publication-quality visualization
- **rtracklayer, GenomicRanges, data.table, dplyr, tidyr, tidyverse** — data wrangling

### Key Analysis Steps (in order)

1. Raw FASTQ QC (`fastqc.sh`)
2. Transcript quantification (`salmon_quant.sh` for ERP161086, or Bambu via Minimap2 for the long-read dataset)
3. TPM normalization and count matrix preparation (`salmon_tx_tpm_counts.r`, `tx_counts_to_tpm.r`)
4. PCA of expression (`pca_tpm.r`)
5. IsoformSwitchAnalyzeR import, filtering, and DTU testing (`isoformswitchanalyzer_DEXSeq.r`)
...
8. Promoter usage analysis restricted to the 86 significant DIU genes (`promoter_usage_analysis_86_genes_12samples.r`)
9. SUPPA2 event generation (`suppa_generate_events_bambu_gtf.sh`) and **targeted** PSI beta regression restricted to transcripts of the 86 significant DIU genes (`diff_beta_reg_sig_diu_tx.r`, `betareg_sex.r`)
10. Visualization: volcano plots, circos/heatmaps, bar plots, GO dot plots (`volcano_isoform_switches.r`, `circos_heatmap_atac_plots_86_genes_12samples.r`, `bar_plot_diu_switch_features.r`, `gprofiler2_dotplot.r`)
11. Independent short-read concordance analysis of the 86 significant DIU genes (`ERP161086/comparig_86_sig_genes_LR_SR_counts.R`)

### Conventions to Preserve

- Output directories are set via `outdir <- "..."` / `OUTPUT_DIR`/`TABLE_DIR`/`FIGURE_DIR` followed by `dir.create()`
...
- The "significant DIU gene set" (86 genes) is the shared anchor across the targeted splicing analysis and the ERP161086 concordance analysis — always confirm which script's `sig_transcripts.txt` / `sig_genes` file is the intended source before modifying filtering logic
...
- **Key guidance for this repo:** Large data files (BAM, FASTQ, GTF, RDS) must never be committed directly; use `.gitignore` and LFS. Hard-coded server paths in scripts should be documented in `README.md` with instructions for adapting to new environments. Retired pipelines (e.g., `PRJNA1206164/`) should be excluded via `.gitignore` rather than left ambiguously tracked.
...
**Skills & Expertise:**
- **IsoformSwitchAnalyzeR**: `importRdata()`, `importIsoformExpression()`, `isoformSwitchTestDEXSeq()`, `analyzeORF()`, `analyzeAlternativeSplicing()`, `analyzeSwitchConsequences()`, `switchPlot()`, `extractTopSwitches()`, `extractConsequenceEnrichment()`
...
- **DEXSeq** as the DTU statistical framework for the long-read pipeline; understanding its assumptions and appropriate use cases
- **Bambu**: long-read transcript discovery, novel isoform annotation, count matrix structure
- **Salmon**: quasi-mapping, `quant.sf` files, TPM computation for the independent short-read concordance cohort
- **SUPPA2**: PSI event types (SE, A3, A5, MX, RI, AF, AL), event file formats, `generate_events.py`
- **Targeted analysis design**: restricting statistical testing (splicing, promoter usage) to a pre-defined significant gene/transcript set, and the tradeoffs vs. genome-wide testing (reduced multiple-testing burden, hypothesis-driven scope, but no discovery of novel loci)
- **Genomics formats**: GTF/GFF3 parsing (`rtracklayer`), BAM/FASTQ handling, UCSC vs. Ensembl chromosome naming
- **Bioconductor ecosystem**: `GenomicRanges`, `BSgenome`, `biomaRt`, `org.Hs.eg.db`
- **Cross-platform concordance analysis**: comparing long-read vs. short-read effect sizes (log2 fold-change / log2(TPM+1) deltas), directional concordance, effect-size ratio metrics
- **Promoter analysis**: TSS identification, promoter window definition, differential promoter usage
- **GO/pathway enrichment**: `gprofiler2::gost()`, interpreting enrichment results in the context of neurodegeneration
...
- **Effect-size metrics**: log2(TPM+1) deltas, effect-size ratios (min/max of absolute effect sizes) for cross-platform concordance
...
- **Statistical modeling**: beta regression (`betareg`, `glmmTMB`) for bounded response variables (PSI, IF), interpreting coefficients and p-values, multiple testing correction (BH/FDR)
...
- **Scatter/concordance plots**: long-read vs. short-read effect size comparisons, same-direction/discordant point styling
...
- **Methods section writing**: precise, citation-ready descriptions of DTU analysis steps, targeted splicing analysis rationale, cross-platform concordance methodology, statistical thresholds, genome versions, and tool versions
- **Data sources commonly used in this field:**
  - **Synapse/ADSP** (syn52047893, syn52065646): AD Knowledge Portal long-read RNA-seq data
  - **ENA** (ERP161086): independent short-read postmortem prefrontal cortex cohort used for concordance validation
...
4. **Preserve conventions**: Respect existing conventions in the codebase — `outdir`/config-variable patterns, RDS checkpointing, CT/AD group labels, and column naming — unless explicitly asked to refactor them.
5. **No silent changes**: Never change behavior, statistical thresholds (e.g., q-value cutoffs, dIF cutoffs, |ΔPSI| cutoffs, effect-size ratio thresholds), or data filtering logic without explicitly flagging the change and explaining the reasoning.
...
8. **Ask before assuming**: If a prompt is ambiguous about which dataset (`ERP161086` vs. `syn52047893`/`syn52065646`), which comparison (CT vs. AD), or which pipeline stage (whole-pipeline DTU vs. targeted splicing vs. concordance analysis) is being discussed — ask for clarification before proceeding.
