...
BASE_DIR   <- "/home/AD.UNLV.EDU/Shared_Data/AlternativeSplicing/ERP161086/results"
OUTPUT_DIR <- file.path(BASE_DIR, "go_kegg_enrichment")
...
if (nrow(as.data.frame(ego)) == 0) {
  ...
  print(dotplot(ego, showCategory = 20,
                title = "GO Biological Process — ERP161086 Significant SUPPA2 Events"))
  dev.off()
...
if (nrow(as.data.frame(ekegg)) == 0) {
  ...
  print(dotplot(ekegg, showCategory = 20,
                title = "KEGG Pathway Enrichment — ERP161086 Significant SUPPA2 Events"))
  dev.off()
