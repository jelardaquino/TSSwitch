...
#           Sex is included as a covariate. Results are filtered by BH-adjusted
...
LONG_READ_SIG  <- "/home/AD.UNLV.EDU/Shared_Data/AlternativeSplicing/syn25671149/jaquino_analysis/tmp/suppa/suppa_beta_regression_significant_results.tsv"
OUTPUT_DIR     <- BASE_DIR
...
# Harmonize group label: rename "CTR" -> "CT" for pipeline consistency
sample_info$group <- ifelse(sample_info$group == "CTR", "CT", sample_info$group)
...
# ── Beta regression: PSI ~ group + sex, per splicing event ───────────────────
...
write.table(res_sig,
  file = file.path(OUTPUT_DIR, "PRJNA1206164_sig_events.tsv"),
  sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
...
event_type_plot <- ggplot(event_counts,
  ...
  labs(
    ...
    title = "Significant SUPPA2 Events by Type — PRJNA1206164"
  ) +
