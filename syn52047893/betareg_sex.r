...
FIGURE_DIR <- file.path(BASE_DIR, "figures", "suppa_beta_regression_sex")
TABLE_DIR <- file.path(BASE_DIR, "tables", "suppa_beta_regression_sex")
...
run_beta_one_event <- function(df_event, meta, sample_cols) {
  ...
  # This tests the overall contribution of condition (main effect + interaction).
...
process_event_type <- function(event_type, sig_tx, meta, gene_symbols) {
  ...
  readr::write_tsv(
    tx_matches,
    file.path(TABLE_DIR, paste0(event_type, "_transcript_matches_sex.tsv"))
  )
  ...
  if (nrow(beta_results) == 0) {
    readr::write_tsv(
      beta_results,
      file.path(TABLE_DIR, paste0(event_type, "_beta_results_sex.tsv"))
    )
  ...
  readr::write_tsv(
    sig_beta_events,
    file.path(TABLE_DIR, paste0(event_type, "_sig_beta_events_sex.tsv"))
  )

  readr::write_tsv(
    sig_event_tx,
    file.path(TABLE_DIR, paste0(event_type, "_sig_beta_event_transcript_matches_sex.tsv"))
  )

  readr::write_tsv(
    event_labels,
    file.path(TABLE_DIR, paste0(event_type, "_event_label_lookup_sex.tsv"))
  )

  readr::write_tsv(
    plot_df,
    file.path(TABLE_DIR, paste0(event_type, "_sig_beta_events_plot_data_sex.tsv"))
  )
  ...
  if (nrow(sig_beta_events) > 0) {
    plot_sig_events(
      plot_df = plot_df,
      event_type = event_type,
      figure_path = file.path(FIGURE_DIR, paste0(event_type, "_sig_beta_events_sex.png"))
    )
  }
...
readr::write_tsv(
  all_matches,
  file.path(TABLE_DIR, "all_event_transcript_matches_sex.tsv")
)

readr::write_tsv(
  all_beta_results,
  file.path(TABLE_DIR, "all_beta_results_sex.tsv")
)

readr::write_tsv(
  all_sig_beta_events,
  file.path(TABLE_DIR, "all_sig_beta_events_sex.tsv")
)

readr::write_tsv(
  all_sig_event_tx,
  file.path(TABLE_DIR, "all_sig_beta_event_transcript_matches_sex.tsv")
)

readr::write_tsv(
  all_labels,
  file.path(TABLE_DIR, "all_event_label_lookup_sex.tsv")
)

readr::write_tsv(
  all_plot_df,
  file.path(TABLE_DIR, "all_sig_beta_events_plot_data_sex.tsv")
)
...
if (nrow(all_plot_df) > 0) {
  ...
  combined_plot <- ggplot(all_plot_df, aes(x = condition, y = psi, shape = sex)) +
    ...
    scale_color_manual(values = c("CT" = "turquoise", "AD" = "salmon")) +
    scale_shape_manual(values = c("Male" = "\u2642", "Female" = "\u2641")) +
    stat_summary(
      aes(group = condition),
      fun = mean,
    ...
    facet_wrap(~ plot_label, scales = "fixed", ncol = 3) +
    scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.25)) +
    theme_bw(base_size = 12) +
    theme(
  ...
  ggsave(
    filename = file.path(FIGURE_DIR, "all_sig_beta_events_sex.png"),
    plot = combined_plot,
    width = 15,
    height = 10,
    dpi = 300,
...
readr::write_tsv(
  summary_table,
  file.path(TABLE_DIR, "summary_by_event_type_sex.tsv")
)
